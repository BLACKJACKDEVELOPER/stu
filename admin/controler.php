...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
  ...
  $file = $_POST['file'];
  // save
  new db('UPDATE controler SET
  fullname="'.$fullname.'",
  email="'.$email.'",
  password="'.$password.'",
  number_phone="'.$nphone.'" WHERE fullname="'.$_GET['name'].'";');

  if ($file != "false") {
    $file = base64_decode($file);
    $name = rand().'.jpg';
    file_put_contents('../assets/profile/'.$name,$file);
    new db('UPDATE controler SET profile = "'.$name.'" WHERE fullname="'.$_GET['name'].'";');
  }

}

$me = new db('SELECT * FROM controler WHERE fullname="'.$_GET['name'].'";');
$me = $me->fetch_assoc();
?>
<!DOCTYPE html>
